...
export class ScheduleAssignmentViewTopicResultResponse {
  private baseUrl = 'https://www.abacustrainer.com/'; // Replace with your API base
    ...
    async getAllocatedResult(examRnm: string): Promise<AllocatedResultResponse> {
      ...
      const response = await CapacitorHttp.request({
        method: 'POST',
        url: `${this.baseUrl}apicalls/Index/getScheduleAssignmentTopicPraticeResult`,
     ...
     async getAllocatedAssignmentResult(examRnm: string): Promise<AllocatedResultResponse> {
      ...
      const response = await CapacitorHttp.request({
        method: 'POST',
        url: `${this.baseUrl}apicalls/Index/getAllocatedAssignmentTopicPraticeResult`,
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        data: body.toString(),
      });
